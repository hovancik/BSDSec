class EmailProcessor
  def self.process(email)
    ...
    when "announce@openbsd.org"
        a = Article.create(title: email.subject, body: email.body, from: email.from.to_s)
        a.tags.create name: 'openbsd'
        $client.update(email.subject+ " #OpenBSD")
    when "freebsd-announce@freebsd.org"
        b = Article.create(title: email.subject, body: email.body, from: email.from.to_s)
        b.tags.create name: 'freebsd'
        $client.update(email.subject+ " #FreeBSD")
    when "security-officer@netbsd.org"
        c = Article.create(title: email.subject, body: email.body, from: email.from.to_s)
